...
import qualified Data.Aeson as JSON (decodeStrict, encode, Value, Value(Number), Value(String), Value(Bool))
...
import qualified Data.ByteString.Base64.URL as B64URL (encodeUnpadded, decodeUnpadded)
...
fromToken publicKey token = do
    ...
    if  RSA.verify hashAlg publicKey payload decodedSignature then
